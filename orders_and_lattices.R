...
relational.product=function(X,Y){  ### berechnet Relationenprodukt von X und Y : X;Y:= (p,q) : exists r: pXr und rYq,  vgl. z.B. http://www.math.tu-dresden.de/~ganter/inf2005/folien/Relationenslides.pdf
  m=dim(X)[1]
  n=dim(X)[2]
  o=dim(Y)[2]
  ans=array(0,c(m,o))
...
neighbour.incidence=function(I){     #berechnet Nachbarschaftsrelation (covering relation) zu einer homogenen Relation I)
  n=dim(I)[1]
...
transitive.hull=function(I){   #  berechnet transitive Huelle einer homogenen Relation  
  m=dim(I)[1]
...
trr=function(I){        # Hilfsfunktion zur Funktion tr, siehe unten
  I1=I*lower.tri(I)
  I2=I*upper.tri(I)
  ans=pmax(neighbour.incidence(I1),neighbour.incidence(I2))
return(ans)}
...
next.closure=function(cop,bg,n=dim(bg$context)[2],Nmax=700000){   # Next closure Algorithmus zur Berechnung aller Huellen eines gegebenen Hüllenoperators cop vgl. z.B. Ganter. Diskrete Mathematik: geordnete Mengen S.86
  A=rep(0,n)
  A=cop(A,bg)
...
oplus=function(A,i,cop,bg){cop(plus(A,i),bg)}  ## Hilfsfunktion fuer Next closure Algorithmus



PHI=function(A,bg){  ##  Ableitungsoperator Phi
  i=which(A==1)
...
PSI=function(B,bg){    ## Ableitungsoperator Psi
  i=which(B==1)
  temp=as.matrix(bg$context[i,])
  dim(temp)=c(length(i),dim(bg$context)[2])
  S=colSums(temp)
  j=which(S==length(i))
  ans=rep(0,dim(bg$context)[2])
  ans[j]=1
return(ans)}


L=function(A,bg=bg){   ## Operator L vgl. Ganter, Wille 1996 S. 80 ff
   temp=A
   if(all(A==0)){temp=H.attr(A,bg)}
   else{
     index=which(A==1)
     for(k in index){
       temp2=A;temp2[k]=0
       
       temp=pmax(temp,H.attr(temp2,bg))
     }
   }
   return(temp)
}

L.infty=function(A,bg=bg){   ## Operator L_infty vgl. Ganter, Wille 1996 S. 85 ff
...
H.attr=function(A,bg){PSI(PHI(A,bg),bg)} ## Berechnet zu Merkmalsmenge A (gegeben durch A[i]=1 iff Merkmal i ist in Menge A, 0 sonst) deren Hülle PSI(PHI(A))
	H.obj=function(A,bg){PHI(PSI(A,bg),bg)} ## Berechnet zu Gegenstandsmenge A (gegeben durch A[i]=1 iff Gegenstand i ist in Menge A, 0 sonst) deren Hülle PHI(PSI(A))
...
concept.lattice=function(context,compute.extents=TRUE,stepsize=20000){### Berechnet Begriffsverband zu Kontext.
   ...
   bg=list(context=context,stepsize=stepsize)
   ans=list()
   ans$intents=next.closure(H.attr,bg)
   m=dim(ans$intents)[1]
   n=dim(context)[1]
   ans$concepts=rep("",m)
   if(compute.extents){ ans$extents=matrix(FALSE,nc=n,nr=m)
      for(k in (1:m)){
        ans$extents[k,]=PHI(ans$intents[k,],bg)
        ans$concepts[k]=paste("{",paste((rownames(bg$context))[which(ans$extents[k,]==1)],collapse=","),"}   {",paste((colnames(bg$context))[which(ans$intents[k,]==1)] ,collapse=","),"}",collapse="")
      }
      
   }
   else{
     for(k in (1:m)){
        ans$concepts[k]=paste("{",paste((colnames(bg$context))[which(ans$intents[k,]==1)],collapse=","),"}",collapse="")
     }
   }
...
random.context=function(nrow=20,ncol=10,prob=0.5){matrix(runif(nrow*ncol)<=prob,nrow=nrow,ncol=ncol)*1}  ### Funktion erzeugt zufälligen Kontext
...
col.reduce=function(X){  ## reduziert Kontext um reduzible Merkmale
  ans=NULL
  m=dim(X)[1]
  n=dim(X)[2]
  for(k in (1:n)){
...
extent.opt.a=function(X,gen.index,v,binary.variables="afap"){##  extentopt: Version, wie in VL beschrieben
...
extent.opt.b=function(X,gen.index,v,binary.variables="afap"){##  extentopt: Version, wie in TR 209, S.23 beschrieben, nur Ungleichungen (21) verschärft
...
extent.opt.c=function(X,gen.index,v,binary.variables="afap"){##  extentopt: Version, wie extent.opt.b, nur, dass Matrix, die Ungleichungsnebenbedingungen enthält, direkt sofort als simple triplet Matrix erzeugt wird
...
k.extent.opt.b=function(X,gen.index,v,binary.variables="afap",K){##  extentopt: Version, wie TR 209, S.23 beschrieben, nur Ungleichungen (21) verschärft
...
test.extent.opt=function(method,M=10,N=5,p=0.5,M1=5,binary.variables="afap"){  ## testet auf Korrektheit der Funktion extent.opt
  X=random.context(M=M,N=N,p=p)#runif(M*N)>=0.5;dim(X)=c(M,N)
 ...
 temp=method(X,gen.index=i,v=v,binary.variables=binary.variables)
...
min.k.attr.generated=function(extent,intent,X){  # Berecchnet für Begriff gegeben durch Umfang extent und Inhalt intent das maximale k, für das der Begriff k-Merkmalserzeugt ist (Kontext X muss ebenfalls mit übergeben werden)
  m=dim(X)[1]
  n=dim(X)[2]
  model=k.extent.opt.b(X,(1:m),(1:m),K=dim(X)[2])
...
min.k.obj.generated=function(extent,intent,X){min.k.attr.generated(intent,extent,t(X))} # Berecchnet für Begriff gegeben durch Umfang extent und Inhalt intent das maximale k, für das der Begriff k-MGegenstandserzeugt ist (Kontext X muss ebenfalls mit übergeben werden)
...
subgroup.discovery.fca.milp=function(dat,target,target.class,nrep,heuristic,remove.full.columns=TRUE,clarify.cols=FALSE,reduce.cols=FALSE,weighted=FALSE,small){ #Fuehrt Subgroup Discovery durch (erzeugt nur MILP, das dann noch mit z.B. gurobi(...) gelöst werden muss
   XX=dat
   XX[[target]]=NULL
   print(objects(XX))
...
extent.VC=function(X,additional.constraint=TRUE){  # Berechnet VC-Dimension eines Kontextes X
  m=dim(X)[1]
  n=dim(X)[2]
  ans=list()
...
local.object.VCdims=function(X,indexs=(1:dim(X)[1]),outputflag,timelimit,pool=FALSE,transpose=TRUE,additional.constraint=TRUE){
...
local.object.VCdims.Hannah=function(X,indexs=(1:dim(X)[1]),outputflag,timelimit,pool=FALSE,transpose=TRUE,additional.constraint=TRUE){
  m=dim(X)[1]
  n=dim(X)[2]
  ans=list()
  vcdims=rep(0,m)
